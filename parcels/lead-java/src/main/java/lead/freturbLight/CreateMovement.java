...
public class CreateMovement {
    ...
    static double calculateMovement(String ST45, String EFETCENT) throws Exception {
        if (!EFETCENT.equals("NN") && !EFETCENT.equals("") && !EFETCENT.equals("00")) {
            int employees = Integer.parseInt(EFETCENT);
            MovementFunction movementFunction = movementFunctionMap.get(ST45);
            return (selectCorrectFunction(movementFunction, employees));
        }
        return 0;
    }
    ...
    public static double calculateMovementTest(int st8, int EFETCENT) throws Exception {
        int count = 0;
        double movments = 0;
        for (Categorisation.St45Class st45Class : Categorisation.st45Map.values()) {
            if (st8 == st45Class.st8) {
                movments += selectCorrectFunction(movementFunctionMap.get(st45Class.st45), EFETCENT);
                count++;
            }
        }
        if (count != 0) {
            return movments / (count);
        }
        return 0;
    }
